...
const NavComponents = () => {
  ...
  const maxMileage: number = Math.max(
    ...announcements.value.map((car) => car.mileage)
  );
  const maxPrice: number = Math.max(
    ...announcements.value.map((car) => car.price)
  );
  const filterMileage = State<{ min: number; max: number }>({
    min: 0,
    max: maxMileage,
  });
  const filterPrice = State<{ min: number; max: number }>({
    min: 1000,
    max: maxPrice,
  });
    ...
    <NavStyle>
      <div className='content'>
        <div className='selectOptions'>
          <h2> Marca </h2>
          <ul>
            {[...new Set(announcements.value.map((car) => car.brand))].map(
              (brand) => (
                <li
                  key={brand}
                  className={
                    filters.value.brands.includes(brand) ? 'selected' : ''
                  }
                  onClick={() => {
        ...
        <div className='selectOptions'>
          <h2> Modelo </h2>
          <ul>
            {[...new Set(announcements.value.map((car) => car.model))].map(
              (model) => (
                <li
                  key={model}
                  className={
                    filters.value.models.includes(model) ? 'selected' : ''
                  }
                  onClick={() => {
                    if (filters.value.models.includes(model)) {
                      filters.set((prev) => {
                        const newFilters = { ...prev };
                        newFilters.models = newFilters.models.filter(
                          (v) => v !== model
                        );
        ...
        <div className='selectOptions'>
          <h2> Cor </h2>
          <ul>
            {[...new Set(announcements.value.map((car) => car.color))].map(
              (color) => (
                <li
                  key={color}
                  className={
                    filters.value.colors.includes(color) ? 'selected' : ''
                  }
                  onClick={() => {
        ...
        <div className='selectOptions'>
          <h2> Ano </h2>
          <ul>
            {[...new Set(announcements.value.map((car) => car.year))].map(
              (year) => (
                <li
                  key={year}
                  className={
                    filters.value.years.includes(year) ? 'selected' : ''
                  }
                  onClick={() => {
                    if (filters.value.years.includes(year)) {
                      filters.set((prev) => {
                        const newFilters = { ...prev };
                        newFilters.years = newFilters.years.filter(
                          (v) => v !== year
                        );
        ...
        <div className='selectOptions'>
          <h2> Combustível </h2>
          <ul>
            {[...new Set(announcements.value.map((car) => car.fuel_type))].map(
              (fuelType) => (
                <li
                  key={fuelType}
                  className={
                    filters.value.fuelTypes.includes(fuelType) ? 'selected' : ''
                  }
                  onClick={() => {
                    if (filters.value.fuelTypes.includes(fuelType)) {
                      filters.set((prev) => {
                        const newFilters = { ...prev };
                        newFilters.fuelTypes = newFilters.fuelTypes.filter(
                          (v) => v !== fuelType
                        );
